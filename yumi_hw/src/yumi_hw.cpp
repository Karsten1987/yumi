    ...
    std::vector<transmission_interface::TransmissionInfo> transmissions)
{
  ...
  for(int j=0; j < n_joints_; j++)
  {
    ...
    std::vector<std::string> joint_interfaces
      = transmissions[j].joints_[0].hardware_interfaces_;
    ...
    joint_handle_position = hardware_interface::JointHandle(
        state_interface_.getHandle(joint_names_[j]),
    ...
    double *const lower_limit, double *const upper_limit)
{
  ...
  *upper_limit = std::numeric_limits<double>::max();
  ...
  if (urdf_model != NULL)
  {
    ...
    const boost::shared_ptr<const urdf::Joint> urdf_joint_sitffness
      = urdf_model->getJoint(joint_name + std::string("_stiffness"));
  ...
  if (has_soft_limits)
  {
    const joint_limits_interface::PositionJointSoftLimitsHandle limits_handle_position(
        joint_handle_position, limits, soft_limits);
    ...
    const joint_limits_interface::VelocityJointSoftLimitsHandle limits_handle_velocity(
        joint_handle_velocity, limits, soft_limits);
